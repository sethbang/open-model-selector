...
const MOCK_MODELS: TextModel[] = [
  {
    ...
    is_favorite: true,
    description:
      'Strong reasoning capabilities with the largest context window for detailed analysis.',
  ...
  },
  {
    ...
    is_favorite: false,
    description:
      'The most powerful VL model with superior visual perception, OCR, and multimodal reasoning.',
  ...
  },
  {
    ...
    is_favorite: false,
    description:
      'Optimized for speed and efficiency. Best for quick answers and lightweight tasks.',
...
export const LoadingState: Story = {
  render: () => {
    const [value, setValue] = useState('')
    return (
      <ModelSelector baseUrl="https://httpbin.org/delay/30" value={value} onChange={setValue} />
    )
  },
}
...
export const ErrorState: Story = {
  render: () => {
    const [value, setValue] = useState('')
    return (
      <ModelSelector baseUrl="https://httpbin.org/status/500" value={value} onChange={setValue} />
    )
  },
}
  ...
  render: () => {
    ...
    return (
      <div style={{ marginTop: 300 }}>
        <ModelSelector models={MOCK_MODELS} value={value} onChange={setValue} side="top" />
      </div>
    )
  },
}
...
export const DeprecatedTextModels: Story = {
  render: () => {
    ...
    return (
      <>
        <ModelSelector models={MOCK_DEPRECATED_TEXT_MODELS} value={value} onChange={setValue} />
        <p style={{ marginTop: 8, fontSize: 11, color: '#888' }}>
...
/**
 * In-menu type filter chip.
 *
 * When the catalog contains 2+ distinct model types and the `type` prop is
 * unset, the popover header shows a "Filter: All" chip next to the sort
 * button. Clicking it opens a small menu of types present in the catalog.
 */
    ...
    return (
      <>
        <ModelSelector
          ...
          value={value}
          onChange={setValue}
          placeholder="Click to filter by type..."
        />
        <p style={{ marginTop: 16, fontSize: 12, color: '#888' }}>
          Open the popover and click the "Filter" chip to narrow by type.
        </p>
      </>
    )
  },
...
export const VeniceLive: Story = {
  render: () => {
    ...
    return (
      <>
        <ModelSelector baseUrl={VENICE_BASE_URL} value={value} onChange={setValue} />
        <p style={{ marginTop: 16, fontSize: 12, color: '#888' }}>
          Selected: <code>{value || 'none'}</code>
        </p>
      </>
